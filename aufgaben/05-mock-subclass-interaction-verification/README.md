...
2. Vergleiche den Ist-Stand mit Aufgabe 04: `AccountActivationNotifier` verwendet hier kein Interface, sondern direkt die konkrete Klasse `RemoteNotificationSender`. Über Konstruktor-Injection, also eine Form von Dependency Injection, erhält `AccountActivationNotifier` die Abhängigkeit injiziert. Das bedeutet: Beim Erzeugen von `AccountActivationNotifier` wird ein passendes Objekt übergeben. In der Klasse `App` siehst du, dass `AccountActivationNotifier` dort ein Objekt von `RemoteNotificationSender` erhält.

3. Schreibe in `ex05.accountapp.AccountActivationNotifierTest` einen Unit-Test für `AccountActivationNotifier#notifyAccountActivated(String, String)`. Verwende für die `RemoteNotificationSender`-Abhängigkeit ein Test Double. Dazu eignet sich ein Mock, der den Aufruf von `send(String, String, String)` speichert und später im Test auslesbar macht. Weil es kein Interface gibt, muss der Mock von `RemoteNotificationSender` erben und `send(String, String, String)` überschreiben, damit er in `AccountActivationNotifier` injiziert werden kann. Schreibe den Test im AAA-Stil (Arrange, Act, Assert), wie in der Vorlesung besprochen.

4. Prüfe im Assert-Teil, dass `AccountActivationNotifier` genau die erwartete Benachrichtigung an die Abhängigkeit übergibt. Für den Aufruf
...
5. Reflexion: Welchen Vorteil und welchen Nachteil hat ein Mock als Unterklasse von `RemoteNotificationSender` im Vergleich zu einem Mock, der wie in Aufgabe 04 nur das Interface `AuditLog` implementiert? Woran erkennst du in diesem Beispiel, dass nicht ein Rückgabewert, sondern eine Interaktion im Mittelpunkt steht?
